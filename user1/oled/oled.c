...
{  
	...
	{  
		...
		OLED_WR_Byte (0x00,OLED_CMD);      //ÉèÖÃÏÔÊ¾Î»ÖÃ¡ªÁÐµÍµØÖ·
		OLED_WR_Byte (0x10,OLED_CMD);      //ÉèÖÃÏÔÊ¾Î»ÖÃ¡ªÁÐ¸ßµØÖ·   
		for(n=40;n<128;n++)OLED_WR_Byte(0,OLED_DATA); 
	} //¸üÐÂÏÔÊ¾
}
...
/*
	x:ºá×ø±ê£¬0-127
	y:0-7(ÖÐ¾°Ô°·Ö¸îµÄ)
*s:ÒªÏÔÊ¾µÄ×Ö·û£¨ÖÐÓ¢ÎÄ¾ù¿É£¬Ç°ÌáÊÇÊý×éÖÐ°üº¬µÄºº×Ö£©
*/
void OLED_Print(u8 x, u8 y, char *s)
{
	...
	for(k=0; k<length; k++)
	{
		...
		if(*(s+k) <= 127){//Ð¡ÓÚ128ÊÇASCII·ûºÅ
			...
			for(i=0;i<sizeof(CN16_Msk)/34;i++){//²éÊý×é
				if(Index == CN16_Msk[i].Index){
					//²éÑ¯µ½Õâ¸ö×Ö
					OLED_Set_Pos(x,y);	
					for(t=0;t<16;t++)
						OLED_WR_Byte(CN16_Msk[i].Msk[t],OLED_DATA);//Ð´Èë×ÖÄ£
