...
void OLED_ShowChar(u8 x, u8 y, u8 chr, u8 size1)
{
    ...
    for (i = 0; i < size2; i++)
    {
        ...
        {
            temp = ascii_8_16[chr1][i];   //调用1608字体
        }
        else if (size1 == 24)
        {
            temp = ascii_8_16[chr1][i];   //调用2412字体
        }
        else return;
        for (m = 0; m < 8; m++)    //写入数据
        {
...
void OLED_ShowChar1(uint16_t x, uint16_t y, char c)//写字符
{
    uint8_t i = 0;
    uint8_t j = 0;
    uint16_t count = 0;
...
void OLED_ShowChinese(u8 x, u8 y, u8 num, u8 size1)
{
    ...
    while (size3--)
    {
        ...
        for (i = 0; i < size1; i++)
        {
            if (size1 == 16)
            {
                temp = ascii_8_16[chr1][i];   //调用16*16字体
            }
            else if (size1 == 24)
            {
                temp = ascii_8_16[chr1][i];   //调用24*24字体
            }
            else if (size1 == 32)
            {
                temp = ascii_8_16[chr1][i];   //调用32*32字体
            }
            else if (size1 == 64)
            {
...
void OLED_ScrollDisplay(u8 num, u8 space)
{
    ...
    while (1)
    {
        ...
        if (mm == 100)
        {
            OLED_ColorTurn(nn);//0正常显示，1 反色显示
            if (nn == 0)nn = 1;
            else nn = 0;
            mm = 0;
        }
...
//显示汉字字符串 从字库提取
void OLED_ShowChineseString(uint16_t x, uint16_t y, u8 *ch)
{
    while (*ch != '\0')
    {
        if (*ch < '~')
